...
export const parseXmlFile = (file) => {
    return new Promise((resolve, reject) => {
        ...
        try {
            ...
            // GESTIONE SCONTI (Righe negative nel riepilogo)
            if (totalDiscount < 0) {
            ...
            // 5. (NUOVO) BILANCIAMENTO FINALE ARROTONDAMENTI
            // Calcoliamo la somma matematica dei nostri dati
            const checkSum = parseFloat((finalTaxable + totalVat + finalCassa).toFixed(2));
            // Calcoliamo la differenza col totale vero del documento
            const discrepancy = parseFloat((totalDoc - checkSum).toFixed(2));
            
            // Se c'è una differenza piccola (es. i famosi 4 centesimi), aggiustiamo l'imponibile
            // Questo sistema funziona sia se l'arrotondamento è positivo che negativo.
